...
const DEFAULT_IMAGES: ImageItem[] = [
  {
    ...
    alt: 'Abstract art',
    href: 'https://unsplash.com', // 示例链接
  },
  {
    src: 'https://images.unsplash.com/photo-1755569309049-98410b94f66d?q=80&w=772&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    alt: 'Modern sculpture',
    href: 'https://react.dev', // 示例链接
  },
  {
...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  const normalizedImages = pool.map(image => {
    if (typeof image === 'string') {
      return { src: image, alt: '', href: undefined };
    }
    // 确保 href 被正确传递
  ...
  return coords.map((c, i) => ({
    ...
    href: usedImages[i].href, // 传递 href
  }));
}
...
export default function DomeGallery({
  ...
  const handleItemClick = (element: HTMLElement) => {
    const href = element.parentElement?.dataset.href;
    if (href) {
      // 如果存在链接，在新标签页中打开
            ...
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                <div
                  ...
                  data-src={it.src}
                  data-alt={it.alt}
                  data-href={it.href} // 7. 在 DOM 中添加 data-href 属性
                  data-offset-x={it.x}
                  data-offset-y={it.y}
